...
parfor x=1:length(rho)
    ...
    for j=1:MC
        ...
        while D(j)<K                        % While there are still devices waiting to be decoded...
            ...
            sigma=(rho(x)*(abs(A(:,1)'*G_mmse(:,1))^2))/(rho(x)*I+(vecnorm(A(:,1)',2)^2));
...
%% Saving the Results
Pout_MMSE_SIC=Pout;
avg_Inv_MMSE_SIC=avg_Inv;
avg_SIC_MMSE_SIC=avg_SIC;
save('Results_MMSE_SIC.mat','rho_dB','Pout_MMSE_SIC','avg_Inv_MMSE_SIC','avg_SIC_MMSE_SIC')

%% Plotting the results
fig1=figure(1);
    semilogy(rho_dB,Pout_MMSE_SIC,'LineWidth',1.5)
    hold on
    grid on
    ylim([0 1])
