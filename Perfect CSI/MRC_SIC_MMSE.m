...
parfor x=1:length(rho)
    D=zeros(1,MC);                                      % Number of correctly decoded devicesin each run
    ...
    for j=1:MC
        ...
        if K>N                      % If the number of devices is greater than the number of antennas...
            ...
            for k=1:(K-N)
                ...
                if log2(1+sigma_k)>=rm                  % If the decoding succeeds...
                    D(j)=k;                             % Update the number of correctly decoded devices
            ...
            % MMSE decoding:
            if k==K-N
                temp=inv(eye(N)+rho(x)*G(:,k+1:end)'*G(:,k+1:end));     % Temporary Matrix
                num_Inv(j)=num_Inv(j)+1;                                % Update the number of matrix inversions
                sigma_k=1./real(sum(diag(temp),2))-1;                   % SINR of the devices being decoded
                D(j)=D(j)+sum(log2(1+sigma_k)>=rm);                     % Update the number of correctly decoded devices
            end
        else                                            % Else, traditional MMSE decoding is used
            temp=inv(eye(K)+rho(x)*G'*G);               % Temporary Matrix
            num_Inv(j)=num_Inv(j)+1;                    % Update the number of matrix inversions
            sigma_k=1./real(sum(diag(temp),2))-1;       % SINR of the devices being decoded
            D(j)=sum(log2(1+sigma_k)>=rm);              % Update the number of correctly decoded devices
        end
    end          
...
%% Saving the results
Pout_MRC_SIC_MMSE=Pout;
avg_Inv_MRC_SIC_MMSE=avg_Inv;
avg_SIC_MRC_SIC_MMSE=avg_SIC;
save('Results_MRC_SIC_MMSE.mat','rho_dB','Pout_MRC_SIC_MMSE','avg_Inv_MRC_SIC_MMSE','avg_SIC_MRC_SIC_MMSE')
