...
parfor x=1:length(rho)
    ...
    for j=1:MC
        ...
        while D(j)<K                        % While there are still devices waiting to be decoded...
            if flag==0                      % On the first iteration of the SIC decoding procedure...
                new_G=G(:,:);               % Select the channel vectors of the K devices
            end
            if size(new_G,2)>N              % If the number of remaining devices is greater than the number of antennas...
                G_mmse=new_G(:,1:N);        % Select a NxN matrix with the channel vectors of the N strongest devices
            else                            % Else...
                G_mmse=new_G;               % Utilize the channel vectors of all the remaining devices
            end                
            A=inv(G_mmse*G_mmse'+(1/rho(x))*eye(N))*G_mmse;         % Compute the MMSE linear detector matrix
            num_Inv(j)=num_Inv(j)+1;                                % Update the number of matrix inversions
            sigma=zeros(1,size(G_mmse,2));                          % Initialize the vector of SINR values
            for k=1:size(G_mmse,2)                                  % Attempt to decode multiple devices simultaneously
                ...
                sigma(k)=(rho(x)*(abs(A(:,k)'*G_mmse(:,k))^2))/(rho(x)*I+(vecnorm(A(:,k)',2)^2));
            end
            ...
            if numDec==0
                ...
                G_failed=G_mmse(:,indexNotDec);     % Select their channel vectors
    ...
    avg_SIC(x)=mean(num_SIC);           % Compute the average number of matrix inversions
end

%% Saving the Results
Pout_MMSE_PIC=Pout;
avg_Inv_MMSE_PIC=avg_Inv;
avg_SIC_MMSE_PIC=avg_SIC;
save('Results_MMSE_PIC.mat','rho_dB','Pout_MMSE_PIC','avg_Inv_MMSE_PIC','avg_SIC_MMSE_PIC')

%% Plotting the results
fig1=figure(1);
    semilogy(rho_dB,Pout_MMSE_PIC,'LineWidth',1.5)
    hold on
    grid on
    ylim([0 1])
    
fig2=figure(2);
    plot(rho_dB,avg_SIC_MMSE_PIC,'LineWidth',1.5)
    hold on
    grid on
    
fig3=figure(3);
    plot(rho_dB,avg_Inv_MMSE_PIC,'LineWidth',1.5)
    grid on
