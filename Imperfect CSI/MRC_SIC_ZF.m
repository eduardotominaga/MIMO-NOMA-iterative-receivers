...
parfor x=1:length(rho)
    ...
    D=zeros(1,MC);                                          % Number of correctly decoded MTC packets in each run
    num_Inv=zeros(1,MC);                                    % Number of matrix inversions in each run
    num_SIC=zeros(1,MC);                                    % Number of SIC operations
    for j=1:MC
        G=G_true(:,:,j);                                    % Select the true channel gains
        ...
        G=G(:,ord);                                         % Sort channel gains of the MTC devices in descending order
        A_mrc=A_mrc(:,ord);                                 % Sort the estimated channel gains descending order
        ...
        if K>N                                              % If there are more devices than receive antenna elements...
            for k=1:(K-N)                                   % Start of the MRC-SIC decoding procedure
                ...
                if log2(1+sigma_k)>=r       % If the k-th MTC device is correctly decoded...
                    D(j)=k;                                 % Update the number of correctly decoded MTC packets
                    num_SIC(j)=k;                           % Update the number of SIC operations
                else
                    break;                  % Else, the SIC decoding procedure ends.
                end                               
            end
            if k==K-N       % If the number of remaining users equals the number of antennas, use ZF decoding:                
                G=G(:,k+1:end);                                                     % Channel gains from the remaining devices 
                A_zf=A_mrc(:,k+1:end)*inv(A_mrc(:,k+1:end)'*A_mrc(:,k+1:end));      % ZF Linear detector matrix
                num_Inv(j)=num_Inv(j)+1;                                            % Update the number of matrix inversions
                % Attempt to simultaneously decode of each one of the remaining users using ZF:
                for k3=1:N
                    % Interference from other active MTC devices:
                    I1=0;
                    for k1=1:N
                        if k1~=k3
                            I1=I1+abs(A_zf(:,k3)'*G(:,k1))^2;
                        end                    
                    end
                    ...
                    I2=0;
                    for k2=1:(k-1)
                        I2=I2+abs(A_zf(:,k3)'*G_til(:,k2))^2;
                    end
                    % SINR of the MTC device being decoded:
                    sigma_k=(rho(x)*(abs(A_zf(:,k3)'*G(:,k3))^2))/(rho(x)*I1+rho(x)*I2+(vecnorm(A_zf(:,k3)',2)^2));
                    if log2(1+sigma_k)>=r                       % If the MTC device is correctly decoded...
                        D(j)=D(j)+1;                            % Update the number of correctly decoded MTC packets
                    end
                end
            end
        else            % If there are more receive antennas than devices, traditional ZF decoding is used:            
            G=G_true(:,:,j);                                    % Select the channel gains of the MTC devices
            A=G_hat(:,:,j)*inv(G_hat(:,:,j)'*G_hat(:,:,j));     % ZF Linear detector matrix
            num_Inv(j)=num_Inv(j)+1;                            % Update the number of matrix inversions
            % Attempt to simultaneously decode the K devices using ZF:
            for k=1:K
...
%% Saving the results
Pout_MRC_SIC_ZF_iCSI=Pout;
avg_Inv_MRC_SIC_ZF_iCSI=avg_Inv;
avg_SIC_MRC_SIC_ZF_iCSI=avg_SIC;
save('Results_MRC_SIC_ZF_iCSI.mat','rho_dB','Pout_MRC_SIC_ZF_iCSI','avg_Inv_MRC_SIC_ZF_iCSI','avg_SIC_MRC_SIC_ZF_iCSI')
