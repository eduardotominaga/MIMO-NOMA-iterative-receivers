    ...
    for j=1:MC
        ...
        while D(j)<K
            ...
            for k=1:size(G_zf,2)
                ...
                % decoded:                                        
                sigma(k)=(rho(x)*(abs(A(:,k)'*G_zf(:,k))^2))/(rho(x)*I1+rho(x)*I2+(vecnorm(A(:,k)',2)^2));
            end
            ...
            if numDec==0
                ...
                buffer_CSI_errors=[buffer_CSI_errors G_zf(:,indexDec)-G_h_zf(:,indexDec)];    % Update the buffer of CSI errors
...
%% Saving the Results
Pout_ZF_PIC_iCSI=Pout;
avg_Inv_ZF_PIC_iCSI=avg_Inv;
avg_SIC_ZF_PIC_iCSI=avg_SIC;
save('Results_ZF_PIC_iCSI.mat','rho_dB','Pout_ZF_PIC_iCSI','avg_Inv_ZF_PIC_iCSI','avg_SIC_ZF_PIC_iCSI')

%% Plotting the results
fig1=figure(1);
    semilogy(rho_dB,Pout_ZF_PIC_iCSI,'LineWidth',1.5)
    hold on
    grid on
    ylim([0 1])
