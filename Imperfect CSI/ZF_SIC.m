...
for x=1:length(rho)
    ...
    parfor j=1:MC
        ...
        while D(j)<K
            ...
            A=G_h_zf*inv(G_h_zf'*G_h_zf);           % ZF linear detector matrix
            num_Inv(j)=num_Inv(j)+1;                % Update the number of matrix inversions
            % Compute the interference from the MTC devices waiting
            % to be decoded:
            I1=0;
            ...
            % Compute the SINR of the devices being decoded:                                      
            sigma=(rho(x)*(abs(A(:,1)'*G_zf(:,1))^2))/(rho(x)*I1+rho(x)*I2+(vecnorm(A(:,1)',2)^2));
            ...
            if log2(1+sigma)>=r
                ...
                buffer_CSI_errors=[buffer_CSI_errors G_zf(:,1)-G_h_zf(:,1)];    % Update the buffer of CSI errors
...
%% Saving the Results
Pout_ZF_SIC_iCSI=Pout;
avg_Inv_ZF_SIC_iCSI=avg_Inv;
avg_SIC_ZF_SIC_iCSI=avg_SIC;
save('Results_ZF_SIC_iCSI.mat','rho_dB','Pout_ZF_SIC_iCSI','avg_Inv_ZF_SIC_iCSI','avg_SIC_ZF_SIC_iCSI')

%% Plotting the results
fig1=figure(1);
    semilogy(rho_dB,Pout_ZF_SIC_iCSI,'LineWidth',1.5)
    hold on
    grid on
    ylim([0 1])
