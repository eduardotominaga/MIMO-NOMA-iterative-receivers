...
%% Monte Carlo Simulation - MMSE-SIC Receiver
    ...
    for j=1:MC
        ...
        while D(j)<K
            ...
            if log2(1+sigma)>=r
                ...
                buffer_CSI_errors=[buffer_CSI_errors G_mmse(:,1)-G_h_mmse(:,1)];    % Update the buffer of CSI errors
                new_G=new_G(:,2:end);           % Update the matrix of true channel gains for the next iteration
                new_G_h=new_G_h(:,2:end);   	% Update the matrix of estimated channel gains for the next iteration
                flag=1; 
            else
                break;
            end                                                                                                                                                                       
...
%% Saving the Results
Pout_MMSE_SIC_iCSI=Pout;
avg_Inv_MMSE_SIC_iCSI=avg_Inv;
avg_SIC_MMSE_SIC_iCSI=avg_SIC;
save('Results_MMSE_SIC_iCSI.mat','rho_dB','Pout_MMSE_SIC_iCSI','avg_Inv_MMSE_SIC_iCSI','avg_SIC_MMSE_SIC_iCSI')

%% Plotting the results
fig1=figure(1);
    semilogy(rho_dB,Pout_MMSE_SIC_iCSI,'LineWidth',1.5)
    hold on
    grid on
    ylim([0 1])
    
fig2=figure(2);
    plot(rho_dB,avg_SIC_MMSE_SIC_iCSI,'LineWidth',1.5)
    hold on
    grid on

fig3=figure(3);
    plot(rho_dB,avg_Inv_MMSE_SIC_iCSI,'LineWidth',1.5)
    grid on
